...
export default async function MyPlansPage() {
    ...
    if (!clientProfile || !clientProfile.full_name) {
        // Safe redirect if profile is incomplete or missing, preventing crash on filtering
    ...
    // --- CRASH FIX: Robust Filtering Logic ---
    // Ensure both plan.creator exists AND nested properties are safely accessed.
    const coachPlans = allPlans.filter(plan =>
        plan.creator &&
        plan.creator.role !== 'client'
    );

    const myPlans = allPlans.filter(plan =>
        plan.creator &&
        plan.creator.full_name === clientProfile.full_name
    );
    // --- END CRASH FIX ---
