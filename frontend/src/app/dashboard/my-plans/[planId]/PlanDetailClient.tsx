...
type Plan = {
    ...
    target_carbs?: number;
    target_fats?: number;
    created_by_id: string; // <--- MUST BE PRESENT AND IS NOW A PROP
};
...
// Helper Component for Macros (omitted for brevity, assume correct)
function MacroBar({ label, value, target, color, bg, icon: Icon }: any) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-sm font-bold ${percentage > 100 ? 'text-red-600' : 'text-gray-900'}`}>
                    {value}g
                    {target ? ` / ${target}g` : ''}
                </span>
            </div>
    ...
    useEffect(() => {
        const fetchUserRole = async () => {
            ...
            if (user) {
                ...
                if (profile) {
                    // FIX: Determine isCreator directly from the prop passed from the Server Component
                    setIsCreator(plan.created_by_id === profile.id);
        ...
        fetchUserRole();
    }, [supabase, plan.created_by_id]); // Dependency on the prop is fine
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-200">
                                ...
                                <div className="space-y-3 text-sm">
                                    {mealTypes.map(mealType => {
                                        ...
                                        if (totalMacros.calories === 0) return null; // Prevent division by zero if allMacros is 0
