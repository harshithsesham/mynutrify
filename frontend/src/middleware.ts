...
export async function middleware(req: NextRequest) {
    ...
    const protectedRoutes = [
        ...
        '/find-a-pro',
        '/professionals',
        '/role-selection'  // Back to original path
    ];
    ...
    if (!session && isProtectedRoute) {
        return NextResponse.redirect(new URL('/login', req.url));  // Back to original path
    }
    ...
    if (session && req.nextUrl.pathname === '/role-selection') {
        try {
            const { data: profile } = await supabase
                .from('profiles')
                .select('role')
